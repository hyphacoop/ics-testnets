...
echo "Description=Gaia service"       | sudo tee /etc/systemd/system/$NODE_MONIKER.service -a
...
echo "ExecStart=$HOME/go/bin/$CHAIN_BINARY start --x-crisis-skip-assert-invariants --home $NODE_HOME --p2p.seeds $SEEDS" | sudo tee /etc/systemd/system/$NODE_MONIKER.service -a

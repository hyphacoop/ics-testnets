...
echo "ExecStart=$HOME/go/bin/cosmovisor start --home $NODE_HOME --p2p.persistent_peers $PERSISTENT_PEERS --p2p.seeds $SEEDS" | sudo tee /etc/systemd/system/cv-$NODE_MONIKER.service -a

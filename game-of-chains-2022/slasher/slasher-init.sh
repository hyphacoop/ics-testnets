#!/bin/bash
# Set up an interchain-security-cd service to join the slasher chain.
...
echo "Description=slasher service"       | sudo tee /etc/systemd/system/$NODE_MONIKER.service -a
...
echo "ExecStart=$HOME/go/bin/$CHAIN_BINARY start --x-crisis-skip-assert-invariants --home $NODE_HOME --p2p.persistent_peers $PERSISTENT_PEERS --p2p.seeds $SEEDS" | sudo tee /etc/systemd/system/$NODE_MONIKER.service -a
...
echo "***********************"
echo "After you have updated the genesis file, start the slasher service:"
